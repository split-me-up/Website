...
function getApproved(privateKey, address, callingFunctions) {
    ...
    return new Promise(function (resolve, reject) {
        TokenContract.methods.approve(contractAddress, '999999999999999999999999999999').estimateGas({
            from : address
            ...
            let transaction = {
                ...
                gas : gasPrice + 1000,
                data : TokenContract.methods.approve(contractAddress, '999999999999999999999999999999').encodeABI()
            };
...
function depositSecurity(username, privateKey, address, callingFunctions) {
    return new Promise(function (resolve, reject) {
        ...
        contract.methods.addPrivateKeyHolder(username).estimateGas({from : address}).then(function (gasPrice) {
            let transaction = {
                from : web3.utils.toChecksumAddress(address),
...
function privateKeyRegeneratedTransaction(privateKey, address, username, user1, user2, callingFunctions){
    return new Promise(function (resolve, reject) {
        alert("Using Private Key to gather security deposit");
        contract.methods.privateKeyRetreived(username, user1, user2).estimateGas({from : address}).then(function (gasPrice) {
            let transaction = {
                from : web3.utils.toChecksumAddress(address),
                to :  web3.utils.toChecksumAddress(contractAddress),
                gas : gasPrice + 1000,
                data : contract.methods.privateKeyRetreived(username, user1, user2).encodeABI()
            };
            let signPromise = web3.eth.accounts.signTransaction(transaction, privateKey);
            console.log(signPromise);
            signPromise.then((signedTx) => {
...
function releaseFunds(username, user1, user2, privateKey, callingFunctions) {
    ...
    privateKeyRegeneratedTransaction(privateKey, address, username, user1, user2, callingFunctions);
}
                ...
                if(balance >= DaiToBeSent){
                    checkApproval(address)
                        .then(function (approved) {
                            if(approved){
                                depositSecurity(username, privateKey, address, callingFunctions).then(function () {
                                    resolve(true);
                                });
                            }else{
